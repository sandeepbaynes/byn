...
// checkProcVisibility reports when the daemon cannot see the caller's process
// ancestry, which is what self-authored grants are matched on.
//
// byn lets a caller inject a variable it created itself without an approval,
// and decides "itself" by walking the requesting process's ancestors from the
// daemon. The daemon runs as its own service user, so that walk needs
// /proc entries belonging to the owner to be visible to it. A /proc mounted
// with hidepid=2 (or hidepid=invisible) hides them, and the feature stops
// working — safely, since an unmatched origin just queues an approval, but
// silently, which is the part worth naming. Without this, the symptom is
// "byn keeps asking me to approve variables I just created" with nothing
// connecting it to a mount option.
//
// Not applicable where there is no /proc, or where byn runs everything as one
// user (there is no cross-user read to fail).
func checkProcVisibility(env healEnv, mounts string) (healCheck, bool) {
	c := healCheck{Name: "process ancestry visible to the daemon"}
	if !env.provisioned() || mounts == "" {
		return c, false
	}
	hidden := ""
	for _, line := range strings.Split(mounts, "\n") {
		...
		for _, opt := range strings.Split(f[3], ",") {
			// hidepid=0/off is the permissive default; anything else hides
			// other users' processes from a non-root reader like the daemon.
			if strings.HasPrefix(opt, "hidepid=") &&
				opt != "hidepid=0" && opt != "hidepid=off" {
				hidden = opt
			}
		}
	}
	...
	c.Detail = "/proc is mounted with " + hidden + ", so the daemon cannot see which process created a variable"
	c.Fix = "expect an approval for every variable an agent adds; to restore it, mount /proc with hidepid=0 or add the byn daemon's group to gid= on that mount"
	return c, true
}
