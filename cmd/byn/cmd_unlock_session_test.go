...
// session.SaveToken is a no-op and no session file is written to disk.
//
...
func TestRunUnlock_NoTTY_NoSessionFile(t *testing.T) {
	...
	// Verify: session.SaveToken with ttyRdev()==0 creates no file.
	tok := []byte("deadbeef11223344556677889900aabbccddeeff00112233445566778899aabb")
	err := session.SaveToken(dir, "default", tok)
	if err != nil {
		t.Fatalf("session.SaveToken: %v", err)
	}
	...
	// session.LoadToken must also return nil — no ambient session for non-TTY callers.
	loaded := session.LoadToken(dir, "default")
	if loaded != nil {
		t.Errorf("session.LoadToken with ttyRdev()==0 = %q, want nil", loaded)
	}
}
