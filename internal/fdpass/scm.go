...
func RecvFDs(sock, want int) ([]int, error) {
	...
	if recvflags&unix.MSG_CTRUNC != 0 {
		return nil, fmt.Errorf("fdpass: control data truncated")
	}
	...
	if len(got) != want {
		CloseAll(got)
		return nil, fmt.Errorf("fdpass: expected %d fds, got %d", want, len(got))
	}
...
// CloseAll closes every fd in the slice, ignoring errors. Used both internally
// (on a partial-receive error) and by callers (the daemon closes its received
// copies after the spawner has dup'd them).
