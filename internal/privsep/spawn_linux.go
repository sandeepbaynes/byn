...
type Config struct {
	...
	// (see HelperDestPath()). The helper must be root-owned with cap_setuid,cap_setgid+ep
	// (Linux) or setuid-root (Darwin) so it can drop the child to ExecUID/ExecGID.
	HelperPath string
...
// linuxSpawner is the Linux implementation of Spawner.
...
func NewSpawner(cfg Config) Spawner {
	return &linuxSpawner{cfg: cfg}
}

// Spawn implements Spawner for Linux.
//
// Security properties:
...
func (s *linuxSpawner) Spawn(req SpawnReq) (int, error) {
	if len(req.Argv) == 0 {
