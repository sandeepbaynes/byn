...
**v0.6.2** · by Sandeep Baynes · [github.com/sandeepbaynes/byn](https://github.com/sandeepbaynes/byn) ·
...
# `byn` would not be a command you can run. `go install` takes one binary at a
# time, so the privileged spawn helper comes separately; the editor is fetched
...
the `sudo byn setup` step — they ask for your password during the install. The
Go toolchain runs no install hook, so a `go install` cannot provision itself;
`byn setup` is the first moment byn has root, and it is what puts `byn` on the
system PATH.
...
- **Privilege separation, engaged by provisioning:** the daemon runs as a dedicated `_byn` service user and trusted-pinned `byn exec` children as `_byn-exec` (a three-UID model: owner ≠ `_byn` ≠ `_byn-exec`). `sudo byn setup` provisions it, and the install script and system packages run that for you. It raises the bar to root — it does **not** defend against root / `CAP_SYS_PTRACE`. On an unprovisioned machine the daemon and exec children run at your UID and the same-UID env-sniff / daemon-ptrace holes remain; `byn doctor` says which you have. `[security] privsep` is the switch that engages it: `byn exec` asks the daemon, and the daemon answers from that key, so a provisioned machine with the flag unset still runs exec children at your UID. `byn setup` sets it.
