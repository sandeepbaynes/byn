...
1. **Agent**: lists current state.
   ...
   byn status --json
   byn project list --json
   byn --project example-project env list --json
   byn --project example-project list --json
   ```
...
2. **User (before the agent is active)**: import any existing `.env`
   ...
   ```sh
   byn --project example-project import .env.local
...
3. **Agent**: writes a runner that uses byn for any command that
   ...
   ```sh
   #!/usr/bin/env bash
   exec byn --project example-project exec -- "$@"
   ```
