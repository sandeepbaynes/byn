...
- **Weaknesses (just as honestly):**
  ...
  - **The same-UID ceiling is real in v0.2.0.** A code-executing
    process as your UID can still ptrace the *unlocked* daemon or read
    an injected child's environ. Privilege separation (daemon and exec
    children on their own service UIDs) is in progress for the next
    release — until it ships, OS-level isolation of untrusted code is
    the recommended companion control.
