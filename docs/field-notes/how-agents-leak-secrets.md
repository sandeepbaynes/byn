...
leak happens, what byn does about it **today (v0.7.0)**, where byn
...
- **Now (opt-in) + coming:** privilege separation — opt-in since v0.3.0
  ...
  macOS, `/proc/<pid>/environ` on Linux). **v0.7.0** spawns that child in your
...
| Vector | v0.7.0 (unprovisioned) | v0.7.0 provisioned (`sudo byn setup`) | Planned |
|---|---|---|---|
